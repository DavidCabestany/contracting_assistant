...
def verify_token(
    ...
    try:
        ...
        if current_time <= exp_time + timedelta(
            ...
            raise HTTPException(
                status_code=401, detail="Verify Token has expired"
            )
    except JWTError:
        raise HTTPException(
...
def decode_token(token: str):
    try:
        ...
        raise HTTPException(
            status_code=401, detail="Decode Invalid token claims"
        )
    except jwt.JWTError as e:
        if "Signature verification failed" in str(e):
            raise HTTPException(
                status_code=401, detail="Decode Invalid token signature"
            )
        elif "Invalid audience" in str(e):
            raise HTTPException(
                status_code=401, detail="Decode Invalid token audience"
            )
        elif "Invalid issuer" in str(e):
            raise HTTPException(
                status_code=401, detail="Decode Invalid token issuer"
            )
        else:
...
def renew_token(current_token: str):
    try:
        ...
        if current_time > exp_time:
            raise HTTPException(
                status_code=401, detail="Renew Token has expired"
            )
